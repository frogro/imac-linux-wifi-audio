...
# --- Root-Checker: robust (greps brechen Script NICHT mehr ab) ---
cat >"$CHECK_BIN" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
has_wifi_iface() {
  local ifs=""
  ifs="$(ip -o link show 2>/dev/null \
        | awk -F': ' '{print $2}' \
        | grep -E '^(wlan|wl|wifi|wlp|p2p-dev-wl)' || true)"
  [[ -n "$ifs" ]]
}

wifi_ok() {
  if has_wifi_iface; then return 0; fi
  if lsmod | grep -q '^brcmfmac'; then return 0; fi
  if dmesg | grep -qi 'brcmfmac'; then return 0; fi
  return 1
}
...
audio_ok() {
  ...
  if command -v aplay >/dev/null 2>&1 \
     && aplay -l 2>/dev/null | grep -qiE 'CS8409|Cirrus'; then
    return 0
  fi
  if dmesg | grep -qi 'cs8409'; then return 0; fi
  return 1
}
...
# --- Fix-Helper (mit Schnelle Heilung A & B) ---
cat >"$FIX_BIN" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
fix_audio() {
  ...
  if lsmod | grep -q '^snd_hda_codec_cs8409' || dmesg | grep -qi 'cs8409'; then
...
# Service einmalig starten (schreibt status.json / Flags), Timer aktivieren
if ! systemctl start imac-wifi-audio-check.service; then
  echo "⚠️  Root-Check konnte nicht gestartet werden (siehe journalctl -xeu imac-wifi-audio-check.service)." >&2
fi
...
# --- User-Notifier (non-blocking Einrichtung) ---
...
EOF
  ...
  # Versuche, direkt zu aktivieren; wenn keine User-DBus-Session da ist, nur Hinweis ausgeben.
