...
has_wifi_iface(){ ip -o link show 2>/dev/null | awk -F': ' '{print $2}' | egrep -q '^(wlan|wl|wifi|wlp|p2p-dev-wl)'; }
wifi_ok(){ has_wifi_iface && return 0; lsmod|grep -q '^brcmfmac' && return 0; dmesg|grep -qi brcmfmac && return 0; return 1; }
audio_ok(){ lsmod|grep -q '^snd_hda_codec_cs8409' && return 0; [[ -r /proc/asound/cards ]] && egrep -qi 'cs8409|cirrus' /proc/asound/cards && return 0; aplay -l 2>/dev/null|egrep -qi 'CS8409|Cirrus' && return 0; dmesg|grep -qi cs8409 && return 0; return 1; }

cur_kernel="$(uname -r)"; prev_kernel=""; [[ -f "$LAST_FILE" ]] && prev_kernel="$(cat "$LAST_FILE")"
...
# --- Fix-Helper (inline, inkl. Heilung A & B) ---
cat >"$FIX_BIN" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
STATE_DIR="/var/lib/imac-linux-wifi-audio"
...
fix_wifi(){ echo "==> (WiFi) Firmware aktualisieren & Stack neu laden"
...
fix_audio(){ echo "==> (Audio) CS8409 bereitstellen"
...
DO_WIFI=0; DO_AUDIO=0; for a in "$@"; do case "$a" in --wifi) DO_WIFI=1;; --audio) DO_AUDIO=1;; *) usage; exit 2;; esac; done
(( DO_WIFI )) && fix_wifi
(( DO_AUDIO )) && fix_audio
EOF
chmod 0755 "$FIX_BIN"
...
EOF
  ...
  UIDNUM="$(id -u "$U)"; export XDG_RUNTIME_DIR="/run/user/${UIDNUM}"; export DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/${UIDNUM}/bus"
  su - "$U" -c "systemctl --user daemon-reload" || true
